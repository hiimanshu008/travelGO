...
async function FetchData(){
    try{
        let res = await fetch(`http://travelweb-env.eba-mxu6vfyg.ap-south-1.elasticbeanstalk.com/Hotels`)
        res = await res.json()
        console.log(res)
        filter(res)  
    } catch(err){
    ...
    data.forEach((product) => {
       ...
       // Image.src = product.image_link;
        ...
        Booknow.addEventListener("click", ()=>{
                fetch(`http://travelweb-env.eba-mxu6vfyg.ap-south-1.elasticbeanstalk.com/bookHotel/${customerId}/${product.hotelId}`, {
                    method: 'POST',
                    headers: {
...
function handleSearch() {
    ...
    if (searchTerm === "") {
        ...
        fetch(`http://travelweb-env.eba-mxu6vfyg.ap-south-1.elasticbeanstalk.com/searchHotel/${encodeURIComponent(searchTerm)}`)
            .then(response => response.json())
            .then(searchResults => {
